...
try:
    ...
    logging.warning(
        "TensorFlow not available. Neural network features will be limited."
    )
...
try:
    ...
    logging.warning(
        "PyTorch not available. PyTorch neural network features will be limited."
    )
    ...
    def __init__(
        ...
        if framework == "auto":
            if TENSORFLOW_AVAILABLE:
                ...
                raise ImportError(
                    "No deep learning framework available. Install TensorFlow or PyTorch."
                )
        else:
...
class DNNRegressor(NeuralNetworkBase):
    ...
    def _build_pytorch_model(self, input_shape: int) -> nn.Module:
        ...
        class DNNModule(nn.Module):
            def __init__(
                self, input_size, hidden_layers, activation, dropout_rate, batch_norm
            ):
    ...
    def _train_pytorch(self, X, y, validation_split, epochs, batch_size, verbose):
        ...
        for epoch in range(epochs):
            ...
            if verbose and epoch % 10 == 0:
                logger.info(
                    f"Epoch {epoch}: train_loss={train_losses[-1]:.4f}, val_loss={val_loss:.4f}"
                )
        ...
        if self.model is None:
            if self.framework == "tensorflow":
                ...
                raise NotImplementedError(
                    "PyTorch classifier not implemented in this example"
                )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.loss_config = loss_config or {
            "type": "mse" if task_type == "regression" else "crossentropy"
        }

    def _build_keras_model(
        self, input_shape: int, output_shape: int = 1
    ) -> keras.Model:
        ...
        for i, layer_conf in enumerate(self.layer_config):
            ...
            if layer_type == "dense":
                ...
                x = layers.Activation(
                    layer_conf.get("activation", "relu"), name=f"activation_{i}"
                )(x)
        ...
        if self.task_type == "regression":
            ...
            outputs = layers.Dense(output_shape, activation=activation, name="output")(
                x
            )
...
class ConvolutionalNetwork(NeuralNetworkBase):
    ...
    def _build_keras_model(self) -> keras.Model:
            ...
            if len(self.input_shape) == 2:
                ...
                if conv_conf.get("pooling", True):
                    x = layers.MaxPooling1D(
                        pool_size=conv_conf.get("pool_size", 2), name=f"maxpool1d_{i}"
                    )(x)
            else:
                # 2D convolution
                x = layers.Conv2D(
        ...
        model = keras.Model(
            inputs=inputs, outputs=outputs, name="convolutional_network"
        )
    ...
    def _build_keras_model(self) -> keras.Model:
        ...
        for i, dim in enumerate(self.encoding_dims):
            encoded = layers.Dense(
                dim, activation=self.activation, name=f"encoder_{i}"
            )(encoded)
        ...
        for i, dim in enumerate(reversed(self.encoding_dims[:-1])):
            decoded = layers.Dense(
                dim, activation=self.activation, name=f"decoder_{i}"
            )(decoded)

        # Output layer
        outputs = layers.Dense(self.input_dim, activation="sigmoid", name="output")(
            decoded
        )
...
class NeuralNetworkRegressor(FeedForwardNetwork):
    ...
    def _build_layer_config(self) -> List[Dict[str, Any]]:
        ...
        for i, units in enumerate(self.hidden_layers):
            # Dense layer
            config.append(
                {"type": "dense", "units": units, "activation": None, "l2_reg": 0.01}
            )
            ...
            # Activation
            config.append({"type": "activation", "activation": self.activation})

            # Dropout
            if self.dropout_rate > 0:
                config.append({"type": "dropout", "rate": self.dropout_rate})
        ...
        **kwargs,
        ...
        if "callbacks" not in kwargs:
            kwargs["callbacks"] = [
                ...
                ReduceLROnPlateau(
                    patience=10, factor=0.5, min_lr=1e-6, monitor="val_accuracy"
                ),
            ]
