...
class TargetTransformer(BaseEstimator, TransformerMixin):
    ...
    def fit(
        self, y: Union[np.ndarray, pd.Series], X: Optional[np.ndarray] = None
    ) -> "TargetTransformer":
        """
...
class LogTransformer(TargetTransformer):
    ...
    def __init__(self, shift: float = 1.0, base: str = "e", name: str = None):
        ...
        self.shift = shift
        self.base = base
        self._log_func = {"e": np.log, "10": np.log10, "2": np.log2}.get(base, np.log)
        self._exp_func = {
            "e": np.exp,
    ...
    def fit(
        ...
        if np.any(y < -self.shift):
            raise ValueError(
                f"Target contains values less than -{self.shift}. "
...
class BoxCoxTransformer(TargetTransformer):
    ...
    def inverse_transform(self, y_transformed: np.ndarray) -> np.ndarray:
        ...
        if self.standardize:
            y_transformed = self._scaler.inverse_transform(
...
class YeoJohnsonTransformer(TargetTransformer):
    ...
    def inverse_transform(self, y_transformed: np.ndarray) -> np.ndarray:
        ...
        y_original = self._power_transformer.inverse_transform(
            y_transformed.reshape(-1, 1)
        ).ravel()
...
class QuantileTransformer(TargetTransformer):
    ...
    def inverse_transform(self, y_transformed: np.ndarray) -> np.ndarray:
        ...
        y_original = self._quantile_transformer.inverse_transform(
            y_transformed.reshape(-1, 1)
        ).ravel()
...
class RankTransformer(TargetTransformer):
    ...
    def __init__(
        self, method: str = "average", normalize: bool = True, name: str = None
    ):
        """
...
class AutoTargetTransformer(TargetTransformer):
    ...
    def fit(
        ...
        for transformation in self.transformations:
            ...
            try:
                ...
                if self.test_normality:
                    ...
                    score = -np.var(y_transformed) / (
                        np.mean(y_transformed) ** 2 + 1e-10
                    )
        ...
        logger.info(
            f"Selected transformation: {self._transform_params.get('best_transformation', 'none')}"
        )
...
class RobustTargetTransformer(TargetTransformer):
    ...
    def __init__(
        self,
        base_transformer: TargetTransformer,
    ...
    def fit(
        ...
        if len(y_clean) > 0:
            self.base_transformer.fit(
                y_clean, X[~self._outlier_mask] if X is not None else None
            )
        else:
        ...
        logger.info(
            f"Detected {self._transform_params['n_outliers']} outliers "
            ...
            for feature_idx in range(X.shape[1]):
                ...
                for val in unique_values:
                    ...
                    if len(y_subset) > 0:
                        ...
                        n = len(y_subset)
                        smoothed_stat = (
                            n * stat + self.smoothing * self._global_stat
                        ) / (n + self.smoothing)
...
def select_best_transformation(
    ...
    auto_transformer = AutoTargetTransformer(
        transformations=transformations, test_normality=(metric == "normality")
    )
...
def create_transformation_pipeline(
    transformations: List[Union[str, TargetTransformer]],
) -> CompositeTransformer:
    """
...
def example_transformations():
    ...
    robust_transformer = RobustTargetTransformer(
        base_transformer=BoxCoxTransformer(), outlier_method="iqr"
    )
