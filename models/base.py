...
class BaseModel(ABC, BaseEstimator):
    ...
    def __init__(self, name: str = None, random_state: int = None):
        ...
        self.metadata_ = {"created_at": datetime.now().isoformat(), "version": "1.0.0"}
    ...
    def _check_is_fitted(self):
        ...
        if not self.is_fitted:
            raise ValueError(
                f"{self.name} is not fitted yet. Call 'fit' before using this method."
            )
...
class BaseRegressor(BaseModel, RegressorMixin):
    ...
    def predict_interval(
        self, X: Union[np.ndarray, pd.DataFrame], confidence: float = 0.95
...
class EnsembleModel(BaseModel):
    ...
    def __init__(
        self, base_models: List[BaseModel], name: str = None, random_state: int = None
    ):
        """
...
class MetaModel(BaseModel):
    ...
    def fit(
        ...
        X_base, X_meta, y_base, y_meta = train_test_split(
            X_array, y_array, test_size=validation_split, random_state=self.random_state
        )
        ...
        for model in candidates:
            ...
            try:
                scores = cross_val_score(
                    model, X_array, y_array, cv=5, n_jobs=self.n_jobs
                )
                ...
                logger.warning(
                    f"Failed to evaluate {model.__class__.__name__}: {str(e)}"
                )
...
class SimpleLinearModel(BaseRegressor):
    ...
    def __init__(
        self, fit_intercept: bool = True, name: str = None, random_state: int = None
    ):
...
def create_model_pipeline(preprocessor: Any, model: BaseModel, name: str = None) -> Any:
    ...
    return Pipeline([("preprocessor", preprocessor), ("model", model)], memory=None)
