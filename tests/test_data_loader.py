...
class TestTabularDataLoader(unittest.TestCase):
    ...
    def test_filters(self):
        ...
        expected_len = len(self.sample_data[self.sample_data['category'].isin(['A', 'B'])])
    ...
    def setUp(self):
        ...
        self.sample_geodata = gpd.GeoDataFrame({
            ...
            'geometry': [Point(x, y) for x, y in 
                        zip(np.random.uniform(-180, 180, 10), 
                            np.random.uniform(-90, 90, 10))]
        }, crs='EPSG:4326')
...
class TestModelLoader(unittest.TestCase):
    ...
    @unittest.skipIf(not hasattr(unittest.TestCase, 'has_sklearn') or not unittest.TestCase.has_sklearn, 
    ...
    def test_metadata_loading(self):
        ...
        # Load and verify
        loaded_model, metadata = self.loader.load(
            joblib_path, 
...
class TestDatasetSplitter(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.y_imbalanced = pd.Series(
            np.random.choice([0, 1], 1000, p=[0.9, 0.1]),
            name='target'
        )
    ...
    def test_stratified_split(self):
        ...
        for class_label in train_dist.index:
            self.assertAlmostEqual(
                train_dist[class_label], 
                test_dist[class_label], 
                delta=0.05
            )
