...
def advanced_classification_example():
    ...
    logger.info(f"Selected {len(selected_features)} features from {len(features)} original features")
    ...
    with tracker.start_run(run_name='advanced_ensemble') as run:
        ...
        base_models = [
            ...
            ('xgb', XGBClassifier(scale_pos_weight=class_counts[0]/class_counts[1], random_state=42)),
            ('cat', CatBoostClassifier(class_weights='Balanced', random_state=42, verbose=False))
        ]
