...
def advanced_regression_example():
    ...
    poly_features = engineer.create_polynomial_features(
        data[numeric_cols], degree=2, interaction_only=True
    )
    ...
    data_clean = preprocessor.handle_missing_values(
        data_enhanced, numeric_strategy="knn", categorical_strategy="mode"
    )

    # Remove outliers
    data_clean = preprocessor.remove_outliers(
        data_clean, method="isolation_forest", contamination=0.05
    )

    # Scale features
    feature_cols = [col for col in data_clean.columns if col != "price"]
    data_scaled = preprocessor.scale_features(
        data_clean, columns=feature_cols, method="robust"  # Robust to outliers
    )
    ...
    ensemble = EnsembleModel(
        base_models=base_models, meta_model="ridge", use_probabilities=False, cv_folds=5
    )
    ...
    uncertainty_results = uq.ensemble_uncertainty(
        [model for _, model in ensemble.base_models_], X_test, confidence_level=0.95
    )

    # Transform predictions back
    y_pred = target_transformer.inverse_transform(uncertainty_results["predictions"])
    ...
    visualizer.plot_residuals(
        y_test_original, y_pred, plot_type="all"  # Creates multiple residual plots
    )
...
def experiment_tracking_example():
    ...
    tracker = ExperimentTracker(
        experiment_name="house_price_prediction", base_dir="experiments"
    )
    ...
    for exp_config in experiments:
        ...
        with tracker.start_run(run_name=exp_config["name"]) as run:
            ...
            results = pipeline.run(
                data=data, target_column="price", models=[exp_config["model"]]
            )
    ...
    # Get best model
    best_model_info = tracker.get_best_model(metric="rmse", mode="min")
