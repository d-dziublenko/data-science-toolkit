...
def basic_time_series_example():
    ...
    pipeline = TrainingPipeline(task_type="regression", time_series=True)
...
def advanced_time_series_example():
    ...
    date_range = pd.date_range(
        start=data.index.min(), end=data.index.max(), freq="H"  # Hourly frequency
    )
    ...
    windows = {"6h": 6, "12h": 12, "24h": 24, "3d": 72, "7d": 168, "14d": 336}
    ...
    for window_name, window_size in windows.items():
        ...
        # Rolling quantiles
        data[f"roll_q25_{window_name}"] = (
            data[target_col].rolling(window_size).quantile(0.25)
        )
        data[f"roll_q75_{window_name}"] = (
            data[target_col].rolling(window_size).quantile(0.75)
        )
    ...
    def create_fourier_terms(index, period, n_terms):
        ...
        for i in range(1, n_terms + 1):
            features[f"sin_{period}_{i}"] = np.sin(
                2 * np.pi * i * np.arange(len(index)) / period
            )
            features[f"cos_{period}_{i}"] = np.cos(
                2 * np.pi * i * np.arange(len(index)) / period
            )
        return features
    ...
    if HAS_LIGHTGBM:
        ...
        selector_model = LGBMRegressor(n_estimators=100, random_state=42, verbose=-1)
    elif HAS_XGBOOST:
        selector_model = XGBRegressor(n_estimators=100, random_state=42, verbosity=0)
    elif HAS_SKLEARN:
    ...
    feature_importance = pd.Series(
        selector_model.feature_importances_, index=X_train.columns
    ...
    if not models:
        ...
        raise ImportError(
            "Please install at least one of: scikit-learn, XGBoost, or LightGBM"
        )

    # If we only have one model, duplicate it with different parameters
    if len(models) == 1:
        logger.warning(
            "Only one model type available. Creating ensemble with different configurations."
        )
        model_name, model = models[0]
        if hasattr(model, "n_estimators"):
            models.append(
                (f"{model_name}_50", type(model)(n_estimators=50, random_state=42))
            )
            models.append(
                (f"{model_name}_300", type(model)(n_estimators=300, random_state=43))
            )

    # Create ensemble with time series CV
    ensemble = EnsembleModel(
        base_models=models, meta_model="linear", cv_folds=tscv, use_probabilities=False
    )
    ...
    metrics = evaluator.evaluate_time_series(
        y_test, predictions, metrics=["mape", "smape", "mase", "rmse", "mae"]
    )
    ...
    feature_importance_df = pd.DataFrame(
        {
            ...
            "importance": ensemble.base_models_[0][1].feature_importances_[
    ...
    if hasattr(visualizer, "plot_feature_importance_by_category"):
        visualizer.plot_feature_importance_by_category(
            category_importance, title="Feature Importance by Category"
        )
    else:
        # Fallback bar plot
    ...
    if hasattr(visualizer, "plot_multi_step_forecast"):
        ...
        # Plot forecast
        plt.plot(
            future_dates, multi_step_forecasts, label="Forecast", color="red", alpha=0.7
        )
...
def anomaly_detection_time_series():
    ...
    if HAS_SKLEARN:
        ...
        detector = IsolationForest(
            contamination=0.01, random_state=42  # Expect 1% anomalies
        )
    ...
    if hasattr(visualizer, "plot_time_series_anomalies"):
        ...
        plt.scatter(
            X.index[anomaly_mask],
            X["value"][anomaly_mask],
            c="red",
            alpha=0.8,
            s=50,
            label="Anomaly",
