...
from sklearn.metrics import (  # Regression metrics; Classification metrics; Clustering metrics; Other metrics
    ...
    log_loss, matthews_corrcoef, max_error, mean_absolute_error,
...
class ModelEvaluator:
    ...
    def cross_validate(
        ...
        logger.info(
            f"Cross-validation {scoring}: {results['mean']:.4f} (+/- {results['std']:.4f})"
        )
...
class RegressionMetrics:
    ...
    def _calculate_custom_metrics(
        ...
        # Normalized metrics
        if np.mean(y_true) != 0:
            custom_metrics["normalized_rmse"] = np.sqrt(
                mean_squared_error(y_true, y_pred)
            ) / np.mean(y_true)
            custom_metrics["normalized_mae"] = mean_absolute_error(
                y_true, y_pred
            ) / np.mean(y_true)
        ...
        if len(y_true) > 1:
            ...
            custom_metrics["directional_accuracy"] = np.mean(
                true_direction == pred_direction
            )

        # Quantile losses
        for quantile in [0.1, 0.5, 0.9]:
            custom_metrics[f"quantile_loss_{quantile}"] = self._quantile_loss(
    ...
    def evaluate_residuals(
        ...
        # Normality tests
        _, shapiro_p = (
            stats.shapiro(residuals) if len(residuals) < 5000 else (np.nan, np.nan)
        )
        ...
        analysis["normality_tests"] = {
            ...
            "likely_normal": (
                shapiro_p > 0.05 if shapiro_p is not np.nan else dagostino_p > 0.05
            ),
        }
    ...
    def _test_heteroscedasticity(
        self, predictions: np.ndarray, residuals: np.ndarray
    ) -> Dict[str, Any]:
        """
...
class ClassificationMetrics:
    ...
    def _calculate_binary_metrics(
        ...
        if y_proba is not None:
            ...
            binary_metrics["average_precision"] = average_precision_score(
                y_true, pos_proba
            )
    ...
    def calculate(
        ...
        if self.specialized_calculator:
            ...
            if metrics:
                ...
                results.update(
                    {k: v for k, v in standard_metrics.items() if k in metrics}
                )
            else:
...
class ThresholdOptimizer:
    ...
    def optimize(
        ...
        thresholds = np.unique(
            np.concatenate(
                [np.linspace(0, 1, 101), y_proba]  # Include actual probability values
            )
        )
        thresholds.sort()
    ...
    def _calculate_criterion_score(
        self, criterion: str, metrics: Dict[str, float]
    ) -> float:
    ...
    def _check_constraints(
        self, metrics: Dict[str, float], constraints: Dict[str, float]
    ) -> bool:
    ...
    def find_operating_points(
        self, y_true: np.ndarray, y_proba: np.ndarray, targets: Dict[str, float]
        ...
        for target_metric, target_value in targets.items():
            ...
            if best_threshold is not None:
                y_pred_best = (y_proba >= best_threshold).astype(int)
                best_metrics = self._calculate_threshold_metrics(
                    y_true, y_pred_best, y_proba
                )
