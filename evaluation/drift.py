...
class StatisticalDriftDetector(BaseDriftDetector):
    ...
    def fit(
        ...
        self.is_fitted = True
        logger.info(
            f"Fitted statistical drift detector with {len(self.reference_data.columns)} features"
        )
    ...
    def _chi_squared_test(self, current_data: pd.DataFrame) -> DriftResult:
        ...
        for col in self.reference_data.columns:
            if self.feature_types.get(col) == "categorical":
                ...
                ref_freq = np.array(
                    [
                        ref_dist.get(cat, 0) * len(self.reference_data)
        ...
        alpha = 1 - self.confidence_level
        drift_detected = (
            any(p < alpha for p in p_values.values()) if p_values else False
        )
        ...
        for col in self.reference_data.columns:
            if self.feature_types.get(col) == "numerical":
                ...
                try:
                    ...
                    logger.warning(
                        "anderson_ksamp not available, using KS test as fallback"
                    )
...
class DistanceDriftDetector(BaseDriftDetector):
    ...
    def fit(self, reference_data: pd.DataFrame):
        ...
        self.is_fitted = True
        logger.info(
            f"Fitted distance drift detector with {self.distance_metric} metric"
        )
        ...
        for col, ref_hist_data in self.reference_histograms.items():
            if col in current_data.columns:
                ...
                curr_hist, _ = np.histogram(
                    curr_values, bins=ref_hist_data["bin_edges"], density=True
                )
    ...
    def _js_divergence(self, current_data: pd.DataFrame) -> DriftResult:
        ...
        for col, ref_hist_data in self.reference_histograms.items():
            if col in current_data.columns:
                ...
                js_div = 0.5 * np.sum(ref_hist * np.log(ref_hist / m)) + 0.5 * np.sum(
    ...
    def _hellinger_distance(self, current_data: pd.DataFrame) -> DriftResult:
        ...
        for col, ref_hist_data in self.reference_histograms.items():
            if col in current_data.columns:
                ...
                # Hellinger distance
                h_dist = np.sqrt(
                    0.5 * np.sum((np.sqrt(ref_hist) - np.sqrt(curr_hist)) ** 2)
                )
    ...
    def _bhattacharyya_distance(self, current_data: pd.DataFrame) -> DriftResult:
        ...
        # Overall score
        avg_distance = (
            np.mean([d for d in distances.values() if d != np.inf])
            if distances
            else 0.0
        )
...
class ModelPerformanceDriftDetector(BaseDriftDetector):
    ...
    def fit(self, X_reference: pd.DataFrame, y_reference: pd.Series):
        ...
        self.is_fitted = True
        logger.info(
            f"Fitted performance drift detector with reference {self.metric}: {self.reference_performance:.4f}"
        )
    ...
    def detect_drift(
        self, X_current: pd.DataFrame, y_current: pd.Series
    ) -> DriftResult:
        """
        ...
        performance_drop = (self.reference_performance - current_performance) / abs(
        ...
        if len(self.performance_history) >= self.window_size:
            recent_performances = [
                h["performance"] for h in self.performance_history[-self.window_size :]
            ]
            moving_avg = np.mean(recent_performances)
            moving_drop = (self.reference_performance - moving_avg) / abs(
                self.reference_performance
            )
        else:
    ...
    def get_performance_trend(self) -> pd.DataFrame:
        ...
        df["performance_drop"] = (self.reference_performance - df["performance"]) / abs(
            self.reference_performance
        )
        ...
        if len(df) >= self.window_size:
            ...
            df["moving_drop"] = (self.reference_performance - df["moving_avg"]) / abs(
                self.reference_performance
            )
    ...
    def plot_performance_trend(self, save_path: Optional[str] = None):
        ...
        ax1.plot(
            trend_df["timestamp"],
            trend_df["performance"],
            label="Current Performance",
            marker="o",
            alpha=0.7,
        )
        ax1.axhline(
            y=self.reference_performance,
            color="green",
            linestyle="--",
        ...
        if "moving_avg" in trend_df.columns:
            ax1.plot(
                trend_df["timestamp"],
                trend_df["moving_avg"],
                label=f"Moving Average ({self.window_size})",
                linewidth=2,
            )
        ...
        if "moving_drop" in trend_df.columns:
            ax2.plot(
                trend_df["timestamp"],
                trend_df["moving_drop"] * 100,
                label=f"Moving Drop ({self.window_size})",
                linewidth=2,
            )

        ax2.axhline(
            y=self.threshold_drop * 100,
            color="red",
            linestyle="--",
            label="Drift Threshold",
        )
        ax2.axhline(
            y=self.threshold_drop * 50,
            color="orange",
            linestyle="--",
...
class DataDriftDetector:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized drift detector with {len(self.feature_columns)} features"
        )
    ...
    def _compute_reference_statistics(self):
        ...
        for q in [0.25, 0.5, 0.75]:
            self.reference_stats["quantiles"][q] = self.reference_data[
                self.numerical_columns
            ].quantile(q)
    ...
    def detect_drift(
        ...
        method: str = "ks",
        confidence_level: float = 0.95,
        return_feature_scores: bool = True,
    ) -> DriftResult:
        """
        Detect drift between reference and current data.
        ...
        if method not in detection_methods:
            raise ValueError(
                f"Unknown method: {method}. Choose from {list(detection_methods.keys())}"
            )

        # Run detection
        result = detection_methods[method](
            current_data, confidence_level, return_feature_scores
        )
    ...
    def _chi_squared_test(
        ...
        if not self.categorical_columns:
            ...
            return DriftResult(
                drift_score=0.0, drift_type=DriftType.NO_DRIFT, p_value=1.0
            )
        ...
        for col in self.categorical_columns:
            ...
            chi2_stat, p_value = stats.chisquare(
                curr_freq, f_exp=ref_freq * (sum(curr_freq) / sum(ref_freq))
            )
    ...
    def _population_stability_index(
        ...
        for col in self.feature_columns:
            if col in self.numerical_columns:
                ...
                _, bin_edges = pd.qcut(
                    ref_values, q=10, retbins=True, duplicates="drop"
                )

                # Calculate distributions
                ref_dist = (
                    pd.cut(ref_values, bins=bin_edges, include_lowest=True)
                    .value_counts(normalize=True)
                    .sort_index()
                )
                curr_dist = (
                    pd.cut(curr_values, bins=bin_edges, include_lowest=True)
                    .value_counts(normalize=True)
                    .sort_index()
                )
        ...
        return DriftResult(
            ...
            metadata={
                "interpretation": "PSI < 0.1: No drift, 0.1-0.25: Warning, > 0.25: Drift"
            },
        )
    ...
    def _jensen_shannon_divergence(
        ...
        for col in self.numerical_columns:
            ...
            js_div = 0.5 * stats.entropy(ref_hist, m) + 0.5 * stats.entropy(
                curr_hist, m
            )
    ...
    def _maximum_mean_discrepancy(
        ...
        for col in self.numerical_columns:
            ...
            if len(curr_values) > max_samples:
                curr_idx = np.random.choice(
                    len(curr_values), max_samples, replace=False
                )
    ...
    def get_drift_report(
        ...
        for method in methods:
            try:
                result = self.detect_drift(
                    current_data, method=method, return_feature_scores=True
                )
        ...
        if drift_votes:
            ...
            report["summary"]["agreement_rate"] = drift_votes.count(
                most_common_drift
            ) / len(drift_votes)
        ...
        if report["summary"].get("overall_drift_type") == "drift":
            ...
            report["recommendations"].append(
                "Investigate the top drifting features for root cause analysis."
            )
        elif report["summary"].get("overall_drift_type") == "warning":
            report["recommendations"].append(
                "Warning: Some drift detected. Monitor closely."
            )
            report["recommendations"].append(
                "Consider collecting more data for verification."
            )
        else:
            report["recommendations"].append(
...
def example_drift_detection():
    ...
    X_curr, y_curr = make_classification(
        n_samples=1000, n_features=10, n_informative=5, flip_y=0.1, random_state=123
    )
...
def example_specialized_detectors():
    ...
    flip_indices = np.random.choice(
        len(y_test), size=int(0.2 * len(y_test)), replace=False
    )
