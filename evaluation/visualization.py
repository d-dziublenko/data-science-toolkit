...
class ModelVisualizer:
    ...
    def plot_feature_importance(self,
        ...
        if importance_type == 'default':
            ...
            raise NotImplementedError(f"Importance type {importance_type} not implemented in base method")
...
class PerformancePlotter:
    ...
    def plot_cross_validation_scores(self,
        ...
        for i, (model_name, scores) in enumerate(cv_scores.items()):
            ...
            ax.text(i + 1, mean_score, f'{mean_score:.3f}', 
...
class FeatureImportancePlotter:
    ...
    def plot_importance_comparison(self,
        ...
        for model_name, df in importance_dict.items():
            ...
            comparison_data[model_name] = feature_importance.reindex(all_features, fill_value=0)
        
        # Plot heatmap
        plt.figure(figsize=(self.config.figsize[0], len(all_features) * 0.3))
        
        sns.heatmap(comparison_data, cmap='YlOrRd', annot=True, fmt='.3f',
    ...
    def plot_cumulative_importance(self,
        ...
        """
        # Sort by importance
        sorted_importance = feature_importance.sort_values('importance', ascending=False).copy()
        sorted_importance['cumulative_importance'] = sorted_importance['importance'].cumsum()
        sorted_importance['cumulative_importance'] /= sorted_importance['importance'].sum()
        
        # Find number of features for threshold
        n_features_threshold = (sorted_importance['cumulative_importance'] <= threshold).sum()
        
        # Plot
        fig, ax = plt.subplots(figsize=self.config.figsize)
        
        x = range(len(sorted_importance))
        ax.plot(x, sorted_importance['cumulative_importance'], 
               linewidth=self.config.line_width, color='blue')
        
        # Add threshold line
        ax.axhline(y=threshold, color='red', linestyle='--', 
                  label=f'{threshold:.0%} threshold')
        ax.axvline(x=n_features_threshold, color='red', linestyle='--',
        ...
        # Add annotation
        ax.annotate(f'{n_features_threshold} features explain {threshold:.0%} of importance',
...
class ResidualPlotter:
    ...
    def plot_residuals(self,
        ...
        fig, axes = plt.subplots(2, 2, figsize=(self.config.figsize[0] * 1.5, 
                                               self.config.figsize[1] * 1.5))
        
        # 1. Residuals vs Predicted
        ax = axes[0, 0]
        ax.scatter(y_pred, residuals, alpha=self.config.alpha, s=self.config.marker_size)
        ...
        ax = axes[1, 0]
        ax.hist(residuals, bins=30, alpha=self.config.alpha, density=True, label='Residuals')
        
        # Overlay normal distribution
        x = np.linspace(residuals.min(), residuals.max(), 100)
        ax.plot(x, stats.norm.pdf(x, residuals.mean(), residuals.std()), 
               'r-', linewidth=self.config.line_width, label='Normal')
        ...
        try:
            ...
            ax.plot(sorted_pred, spline(sorted_pred), 'r-', linewidth=self.config.line_width)
        except:
            pass
    ...
    def plot_residuals_vs_features(self,
        ...
        for i, feature in enumerate(feature_names):
            ax = axes[i]
            ax.scatter(features[feature], residuals, 
                      alpha=self.config.alpha, s=self.config.marker_size)
            ax.axhline(y=0, color='red', linestyle='--')
...
class ConfusionMatrixPlotter:
    ...
    def plot_multi_class_roc(self,
        ...
        for i, color in enumerate(colors):
            plt.plot(fpr[i], tpr[i], color=color, linewidth=self.config.line_width,
                    label=f'{class_names[i]} (AUC = {roc_auc[i]:.3f})')
        
        # Plot micro-average ROC curve
        plt.plot(fpr["micro"], tpr["micro"], 
                label=f'Micro-average (AUC = {roc_auc["micro"]:.3f})',
                color='deeppink', linestyle=':', linewidth=self.config.line_width + 1)
