...
class DataValidator:
    ...
    def validate(self, 
                 ...
                 feature_columns: Optional[List[str]] = None) -> Tuple[bool, List[ValidationResult]]:
        """
    ...
    def _validate_missing_values(self, data: pd.DataFrame):
        ...
        for col, missing_count in missing_stats[missing_stats > 0].items():
            ...
            self._add_result(
                ...
                details={"missing_count": int(missing_count), "missing_proportion": float(prop)},
                affected_columns=[col]
            )
    ...
    def _validate_statistical_properties(self, data: pd.DataFrame, 
        ...
        for col in numeric_cols:
            ...
            if outliers > 0:
                ...
                self._add_result(
                    ...
                    level=ValidationLevel.INFO if outlier_prop < 0.05 else ValidationLevel.WARNING,
                    message=f"Column '{col}' has {outliers} ({outlier_prop:.1%}) extreme outliers",
                    details={"n_outliers": int(outliers), "outlier_proportion": float(outlier_prop)},
                    affected_columns=[col]
                )
    ...
    def add_custom_rule(self, name: str, rule_func: Callable[[pd.DataFrame], ValidationResult]):
        """
    ...
    def generate_report(self, output_path: Optional[str] = None) -> str:
        ...
        for i, result in enumerate(self.validation_results, 1):
            ...
            if result.affected_rows and len(result.affected_rows) <= 10:
                ...
                report.append(f"    Affected rows: {result.affected_rows[:10]} (and {len(result.affected_rows) - 10} more)")
...
class DataProfiler:
    ...
    def profile_dataset(data: pd.DataFrame, 
        ...
        if target_column and target_column in data.columns:
            profile["target_analysis"] = DataProfiler._analyze_target(data, target_column)
    ...
    def _get_missing_value_summary(data: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            "missing_by_column": {
                col: {
                    "count": int(count),
                    "proportion": float(missing_props[col])
                }
