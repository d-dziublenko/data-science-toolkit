...
class FeatureSelector:
    ...
    def remove_multicollinear_features(
        self, X: pd.DataFrame, threshold: float = 0.8
    ) -> List[str]:
        """
        ...
        logger.info(
            f"Removed {len(to_drop)} multicollinear features (threshold={threshold})"
        )
        return selected
...
class FeatureEngineer:
            ...
            for window in windows:
                for func in funcs:
                    ...
                    if group_col:
                        ...
                        result[feature_name] = (
                            result[col].rolling(window, min_periods=1).agg(func)
                        )
    ...
    def create_date_features(data: pd.DataFrame, date_column: str) -> pd.DataFrame:
        ...
        result[f"{date_column}_is_weekend"] = (
            result[date_column].dt.dayofweek.isin([5, 6]).astype(int)
        )
        result[f"{date_column}_is_month_start"] = result[
            date_column
        ].dt.is_month_start.astype(int)
        result[f"{date_column}_is_month_end"] = result[
            date_column
        ].dt.is_month_end.astype(int)

        # Cyclical encoding for periodic features
        result[f"{date_column}_month_sin"] = np.sin(
            2 * np.pi * result[f"{date_column}_month"] / 12
        )
        result[f"{date_column}_month_cos"] = np.cos(
            2 * np.pi * result[f"{date_column}_month"] / 12
        )
        result[f"{date_column}_day_sin"] = np.sin(
            2 * np.pi * result[f"{date_column}_day"] / 31
        )
        result[f"{date_column}_day_cos"] = np.cos(
            2 * np.pi * result[f"{date_column}_day"] / 31
        )
...
class InteractionFeatures:
    ...
    def create_targeted_interactions(
        self,
        data: pd.DataFrame,
        target_col: str,
        ...
        logger.info(
            f"Created {len(self.created_features)} targeted interaction features"
        )
        return result

    def create_ratio_interactions(
        self,
        data: pd.DataFrame,
        ...
        for num_col in numerator_cols:
            for den_col in denominator_cols:
                if (
                    ...
                    result[feature_name] = data[num_col] / (
                        data[den_col] + min_denominator
                    )
                    self.created_features.append(feature_name)
    ...
    def create_arithmetic_interactions(
        self,
        data: pd.DataFrame,
        columns: List[str],
        ...
        logger.info(
            f"Created {len(self.created_features)} arithmetic interaction features"
        )
        return result

    def _select_top_pairs(
        self, data: pd.DataFrame, pairs: List[Tuple[str, str]], max_pairs: int
    ) -> List[Tuple[str, str]]:
        """
    ...
    def evaluate_interactions(
        ...
        if method == "mutual_info":
            ...
            self.feature_importance = dict(
                zip(self.created_features, model.feature_importances_)
            )
...
class PolynomialFeatures:
    ...
    def fit(
        ...
        logger.info(
            f"Fitted polynomial features: {self.n_input_features} -> {self.n_output_features} features"
        )
    ...
    def create_selected_polynomials(
        self,
        X: pd.DataFrame,
        y: pd.Series,
        max_features: int = 50,
        ...
        logger.info(
            f"Selected {len(selected_new)} polynomial features from {len(new_features)} candidates"
        )

        return result

    def create_interaction_only_features(
        self, X: pd.DataFrame, min_degree: int = 2, max_degree: int = 3
    ) -> pd.DataFrame:
        """
    ...
    def create_custom_polynomials(
        self, X: pd.DataFrame, custom_terms: List[Dict[str, int]]
    ) -> pd.DataFrame:
        """
