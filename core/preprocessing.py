        ...
        if method == 'remove':
            ...
            for col, outlier_mask in outliers.items():
                if col != 'combined' and col in data_treated.columns:
                    ...
                    if data_treated[col].min() > 0:
                        ...
                        logger.warning(f"Cannot apply transformation to {col} due to non-positive values")
            ...
            for col, outlier_mask in outliers.items():
                if col != 'combined' and col in data_treated.columns:
                    ...
                    logger.info(f"Imputed {sum(outlier_mask)} outliers in {col} with {impute_value:.2f}")
            ...
            for col, outlier_mask in outliers.items():
                if col in data.columns:
                    ...
                    summary_data.append({
                        ...
                        'outlier_mean': outlier_values.mean() if len(outlier_values) > 0 else np.nan,
                        'outlier_std': outlier_values.std() if len(outlier_values) > 0 else np.nan,
                        'outlier_min': outlier_values.min() if len(outlier_values) > 0 else np.nan,
                        'outlier_max': outlier_values.max() if len(outlier_values) > 0 else np.nan
                    })
...
class MissingValueHandler:
        ...
        for col in data.columns:
            if data[col].isnull().any():
                ...
                if missing_pct < 0.05:
                    ...
                    missing_types[col] = 'Possibly MAR (at random conditional on observed)'
    ...
    def impute_missing_values(self,
        ...
        if strategy == 'simple':
            ...
            categorical_cols = data[columns].select_dtypes(include=['object', 'category']).columns
            ...
            if len(numeric_cols) > 0:
                ...
                data_imputed[numeric_cols] = iterative_imputer.fit_transform(data[numeric_cols])
            ...
            for col in columns:
                if data[col].isnull().any():
                    ...
                    feature_cols = [c for c in data.columns if c != col and c not in columns]
                    
                    if len(feature_cols) > 0 and train_mask.sum() > 10:  # Need enough training data
                        ...
                        try:
                            ...
                            logger.warning(f"Model-based imputation failed for {col}: {e}")
                            # Fall back to simple imputation
                            if data[col].dtype in ['float64', 'int64']:
                                data_imputed[col].fillna(data[col].median(), inplace=True)
                            else:
                                data_imputed[col].fillna(data[col].mode()[0], inplace=True)
    ...
    def create_missing_indicator(self, data: pd.DataFrame, 
        ...
        if self.missing_patterns:
            ...
            for col, pct in self.missing_patterns['missing_percentages'].items():
                report_data.append({
                    ...
                    'missing_type': self.missing_patterns['missing_types'].get(col, 'Unknown')
                })
